...
model, tokenizer = FastLanguageModel.from_pretrained(
    model_name = '/mnt/models/llama-3-sqlcoder-8b',
    max_seq_length = 2048,
    dtype = None,
    load_in_4bit = True,
)
...
for prompt in prompts:
    ...
    response = tokenizer.decode(x[0], skip_special_tokens=True)
    response = response.replace(table_statements, '').replace('DDL statements:\nassistant', '').replace('\n\n\n\n', '').replace('user\n', '').replace('```sql', '')
